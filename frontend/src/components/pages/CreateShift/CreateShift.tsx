    ...
    try {
      ...
      if (request) {
        ...
        if (request.inviteEmails) {
          await Promise.all(
            request.inviteEmails.map(async (item) => {
              const invitee = await UserAPIClient.getUserByEmail(item);
              await NotificationAPIClient.post({
                assigneeId: invitee.id,
                requesterId: "",
                notificationDescription: `You have been invited to a shift!`,
                notificationTitle: "Shift Invitation",
